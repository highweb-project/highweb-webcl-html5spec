...
void DeviceCpu::load(PassRefPtrWillBeRawPtr<DeviceCpuScriptCallback> passCallback) {
	RefPtrWillBeRawPtr<DeviceCpuScriptCallback> callback = passCallback;
	...
	if (status->getFunctionCode() == function::FUNC_GET_CPU_LOAD) {
		if (callbackList.size() > 0) {
		    ...
		    for (CpuCallbackListIterator it = callbackList.rbegin(); it != itEnd; ++it) {
		        RefPtrWillBeRawPtr<DeviceCpuScriptCallback> callback = *it;
